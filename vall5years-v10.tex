...
\maketitle
\begin{center}
\includegraphics[width = 40mm]{figures/logo_mciu.png}
\end{center}
\newpage
\tableofcontents
\newpage
...
We will explore a number of different algorithms and we will evaluate the goodness of fit of those algorithms with the problem at hand -predict conversion to MCI from healthy subjects. We are being methodologically speaking promiscuous for a reason. There is no a machine learning algorithm that is universally any better than any other. This is related to the no free lunch theorem in search an optimization \cite{wolpert1997no}. It follows that the goal of machine learning is not to seek an universal learning algorithm, instead the goal must be to understand what kinds of distributions are relevant to the real world that an AI agent is experiencing. Thus, we must develop an algorithm to perform well in a particular task and we do this by adding preference hyperparameters \footnote{Hyperparameter is a parameter whose value is set before the learning process begins. By contrast, the values of other parameters are derived via training.} (e.g. regularization) in the algorithm.  
...
%\textit{subject to} y_i -w_ix_i - b \leq \epsilon \\
...
%The problem of overfitting in SVM is tackle via expanding or narrowinfg the margin 'street' with the C parameter (for large C, margins are narrow we could overfit). 
...
%This, in essence is a Random Forest.
...
Even if individual classifiers are weak learners (do slightly better than random guessing) the ensemble can still be a strong classifier, provided there are a sufficient number of weak learners and they are sufficiently diverse. % solo no puedes con amigos si   https://www.youtube.com/watch?v=Ds7tje_Y0CM
...
\captionof{table}{Ensemble methods} \label{tab:ensemblemethods} 
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=1\linewidth]{figures/xvalidationchollet}
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=1\linewidth]{figures/SVM-CM}
...
\begin{figure}[H]
        \centering
        \includegraphics[keepaspectratio, width=0.5\linewidth]{figures/SVM-CV7}
...
The best C and $\gamma$ calculated with Grid-Search are $C= 10.0$ and $\gamma= 0.03125$. %with a score of 0.61
